...
climate_normals  <- climate_normals_raw %>% 
  dplyr::select(name = NAME, date = DATE, temp = `MLY-TAVG-NORMAL`, temp_sd = `MLY-TAVG-STDDEV`,
         precip = `MLY-PRCP-NORMAL`, precip_25 = `MLY-PRCP-25PCTL`, precip_75 = `MLY-PRCP-75PCTL`) %>% 
#   summarise_at(vars(temp,precip), list(mean = ~mean(., na.rm = T), 
#                                     sd = ~sd(., na.rm = T))) %>% 
  mutate(version = '1981-2010')
...
#temp <- 
ggplot(climatology,aes(x = date, y = temp)) +
  geom_col(aes(fill = version), position = 'dodge2') +
  geom_errorbar(aes(ymin = temp - temp_sd/sqrt(26), ymax = temp + temp_sd/sqrt(26)),
...
precip <- 
  ggplot(climatology, aes(x = date, y = precip)) +
  geom_col(aes(fill = version), position = 'dodge2') +
  # geom_errorbar(aes(ymin = temp - temp_sd/sqrt(26), ymax = temp + temp_sd/sqrt(26)),
  #               position = position_dodge2(width = 0.2, padding = 0.8)) +
  scale_fill_manual('', values = c('grey50','grey30')) +
...
plot_grid(precip, temp, rel_widths = c(1, 1.35))
